...
class MailDB:
    ...
    def _to_relative(self, abs_path):
        """Convert an absolute path to a path relative to the DB folder."""
        if not abs_path:
            return abs_path
        try:
            return os.path.relpath(abs_path, self.base_dir)
        except ValueError:
            # Different drive on Windows – store absolute
            return abs_path

    def _to_absolute(self, rel_path):
        """Convert a stored relative path back to absolute."""
        if not rel_path:
            return rel_path
    ...
    def insert_folder(self, name, parent_id, full_path):
        """Store folder path relative to the DB location."""
    ...
    def insert_message(self, folder_id, subject, sender_name, sender_email, date_utc,
        ...
        """Store message file paths relative to the DB location."""
    ...
    def insert_attachment(self, message_id, filename, filepath, size):
        """Store attachment path relative to the DB location."""
...
class PFFExportDialog(QDialog):
    ...
    def _auto_detect_tools(self):
        """Try to auto-detect pffexport and pffinfo."""
    ...
    def _browse_input(self):
        ...
        if path:
            ...
            # Auto-fill target basename from input filename
    ...
    def _build_command(self, tool_path, is_export=True):
        """Build the argument list for pffexport or pffinfo."""
        args = [tool_path]
    ...
    def _start_extract(self):
        ...
        # Run pffinfo first if requested
        ...
        # Build pffexport command
        export_args = self._build_command(export_path, is_export=True)

        # Set working directory to output folder so relative -t works
        try:
            result = subprocess.run(
...
# ----------------------------------------------------------------------
# Scanner thread (unchanged except uses relative paths via MailDB)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.att_model.setHorizontalHeaderLabels(['Filename', 'Message Subject', 'Sender', 'Date', 'Folder', 'Size'])
    ...
    def open_extract_dialog(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            # Offer to open the extracted folder
    ...
    def show_message(self, msg_id):
        ...
        if attachments:
            ...
            for att in attachments:
                ...
                btn.customContextMenuRequested.connect(lambda pos, b=btn: self.attachment_context_menu(pos, b))
    ...
    def att_context_menu(self, pos):
        ...
        if action == open_act:
            ...
            clipboard = QApplication.clipboard()
            clipboard.setText(att['filepath'])
    ...
    def refresh_all_attachments(self):
        ...
        for r in rows:
            ...
            item_size = QStandardItem(size_str)
            item_size.setEditable(False)
            self.att_model.appendRow([item_filename, item_subject, item_sender, item_date, item_folder, item_size])
